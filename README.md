...
2. Forward propagation
    ...
    - Run the activation function ReLU for layer H
    - Apply the second weight and biases for the output layer and run activation function softmax this time.
3. Backward propagation
    - Calculate the differnece in our prediction and the actual label
    - Calculate how much of this differnece is caused by our weight and bias
    - We will also do the same calculations for our hidden layer. After unddoing activation
...
The training data `./data/train.csv` has each row as an image and each column is feature. The number of rows can be in thousands(as much data as we have), we will use 10,000 here to explain. And the columns are just 785: 1 for the label and 784 for the all the 784px in each image.
...
So lets call our input matrix I

Applying our weight and bias is `==>` Z<sup>[1]</sup> = W<sup>[1]</sup>X + b<sup>[1]</sup> 

That is going to be our initial unactivated on the hidden layer.

### 2b. Run the activation function ReLU for layer H
...
We are repeating the same thing for the output layer, but remmeber, the input for this layer is the result of the activation from the previous one A(h)

So our Z<sup>[1]</sup> = W<sup>[1]</sup>X + b<sup>[1]</sup>  becomes Z<sup>[2]</sup> = W<sup>[2]</sup>A<sup>[1]</sup> + b<sup>[2]</sup> . It will have its own set of weights and biases. And at this point, our weight matrices should have reduced to a simple 10X10 since both the input and out layer in this side are 10 neurons.
...
#### 1. $ \underline{dZ^{[2]}} $
The difference between our label(Y) and our prediction A<sup>[2]</sup> is dZ<sup>[2]</sup> and the equation is given by:<br>
`==>` dZ<sup>[2]</sup> = A<sup>[2]</sup> - Y<br>
it is specifically is the gradient of the loss with respect to Z<sup>[2]</sup>
...
### 3b Calculate how much of this differnece is caused by our weight and bias
So after getting  dZ<sup>[2]</sup>, we are going to calculate just how much the weight and bias on the outer layer affected it so we can know which to change and how.

#### 2. $ \underline{dW^{[2]}} $
For the weight on the output layer, the gradient loss(dW<sup>[2]</sup>) is given by
$$
...
#### 3. $ \underline{db^{[2]}} $
the gradient loss of the bias is given by 
$$
...
### 3c. Calculations for our hidden layer. After unddoing activation
#### 4. $ \underline{dZ^{[1]}} $
